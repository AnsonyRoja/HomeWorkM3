...
function problemA() {
  // callback version
  // exerciseUtils.readFile("poem-one/stanza-02.txt", (stanza2) => {
  //   exerciseUtils.blue(stanza2);
  //   exerciseUtils.readFile("poem-one/stanza-03.txt", (stanza3) => {
...
function problemB() {
  ...
  const filePath = "poem-one/wrong-file-name.txt";
  exerciseUtils.promisifiedReadFile(filePath)
    .then((stanza4) => {
      exerciseUtils.blue(stanza4);
    })
    .catch((error) => {
      const errorArray = new Error(error);
      exerciseUtils.magenta(errorArray);
    });
}
